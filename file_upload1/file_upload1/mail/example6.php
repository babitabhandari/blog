...
header('Content-Type: text/csv; charset=utf-8');
header('Content-Disposition: attachment; filename=UnvalidatedEmails.csv');
 ...
 $unvaliq22= "select firstname,lastname,email from  $rec where status='null' AND expire_count=0";
 
 $unvaliqre12=mysql_query($unvaliq22);

  
$unvalidata22 = fopen('php://output', 'w');
 
 
$first = true;
 while($unvalirow23 = array_filter(mysql_fetch_assoc($unvaliqre12))){
 if ($first) {
        fputcsv($unvalidata22, array_keys($unvalirow23));
        $first = false;
    }
   // fputcsv($fp, $row);
fputcsv($unvalidata22, $unvalirow23);
...
$.each($("input[name='new']:not(:checked)"), function(){            
 ...
 if(favorite == 'new1'){
   ...
   $('.unvalidate').css('display','inline-block');
...
$("#epending").attr("disabled", true);
 $("#totalexport").attr("disabled", true);
...
echo $up1;
    ...
    <input  type="submit" value="Export All Emails" name="totalexport" style="background:#3F4C6B;opacity:1.4;color:white;display:none;" class="btn btn-primary totalexport" >
        ...
        <input type="submit" value="Unvalidated Emails" name="newemail" style="background:#3F4C6B;opacity:1.4;color:white;display:none;" class="btn btn-primary newemail">
         <input type="submit" value="Export Unvalidated Emails" name="unvalidate" style="background:#3F4C6B;opacity:1.4;color:white;display:none;" class="btn btn-primary unvalidate">
...
<textarea class="form-control"  name="em" id="text_box" cols="30" rows="10" placeholder="please enter email addresses" class="txt">
<?php 
